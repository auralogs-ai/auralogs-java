...
import ai.auralogs.internal.ErrorCapture;
import ai.auralogs.internal.Logger;
import ai.auralogs.internal.Transport;
import java.util.Map;
import org.jspecify.annotations.Nullable;

/**
 * Static facade for the Auralogs SDK.
 *
 * <p>Call {@link #init(AuralogsConfig)} once at application startup. Emit logs via the {@code
 ...
 *
 * <pre>{@code
 * Auralogs.init(AuralogsConfig.builder().apiKey("...").build());
 * Auralogs.info("user signed in", Map.of("userId", "123"));
 * Auralogs.error("payment failed", Map.of("orderId", "abc"), exception);
 * }</pre>
 */
public final class Auralogs {
   ...
   * previous transport.
   */
  public static synchronized void init(AuralogsConfig config) {
    shutdown();
    Transport t =
        new Transport(
    ...
    if (!shutdownHookRegistered) {
      Runtime.getRuntime()
          .addShutdownHook(new Thread(Auralogs::shutdown, "auralogs-shutdown-hook"));
  ...
  private static Logger require() {
    Logger l = logger;
    if (l == null) {
      throw new IllegalStateException("Auralogs.init(config) must be called before logging");
    }
    return l;
  }
}
