...
class AuralogsSlf4jLoggerTest {
  ...
  @Test
  void loggerFactoryReturnsAuralogsSlf4jLogger() {
    Logger log = LoggerFactory.getLogger("test");
    assertThat(log).isInstanceOf(AuralogsSlf4jLogger.class);
  }
  ...
  @Test
  void debugBelowThresholdDoesNotReachAuralogs() {
    // Default INFO threshold — debug() should be filtered out by AbstractLogger before reaching
    // Auralogs.debug(...). This is the load-bearing DoS-prevention behavior.
    Logger log = LoggerFactory.getLogger("test");
    log.debug("noisy");
    Auralogs.shutdown(); // forces a flush
    assertThat(wm.getAllServeEvents()).isEmpty();
  }

  @Test
  void warnThresholdDisablesInfoAndBelow() {
    AuralogsSlf4jLogger log = new AuralogsSlf4jLogger("test", org.slf4j.event.Level.WARN);
    assertThat(log.isErrorEnabled()).isTrue();
    assertThat(log.isWarnEnabled()).isTrue();
  ...
  void traceThresholdEnablesEverything() {
    AuralogsSlf4jLogger log = new AuralogsSlf4jLogger("test", org.slf4j.event.Level.TRACE);
