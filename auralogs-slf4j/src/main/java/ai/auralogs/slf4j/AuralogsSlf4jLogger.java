...
/**
 * SLF4J {@code Logger} implementation that forwards records to the Auralogs core facade. Returned
 * by {@link AuralogsSlf4jLoggerFactory}; not constructed directly by users.
 *
 ...
 * never reach the Auralogs buffer. This prevents low-severity log floods from filling the in-memory
 * queue in shared logging infrastructure.
 */
public final class AuralogsSlf4jLogger extends AbstractLogger {
  private final Level threshold;

  AuralogsSlf4jLogger(String name) {
    this(name, Level.INFO);
  }

  AuralogsSlf4jLogger(String name, Level threshold) {
    this.name = name;
  ...
  protected String getFullyQualifiedCallerName() {
    return AuralogsSlf4jLogger.class.getName();
  }
  ...
  protected void handleNormalizedLoggingCall(
    ...
    switch (level) {
      case TRACE:
      case DEBUG:
        Auralogs.debug(message, metadata);
        break;
      case INFO:
        Auralogs.info(message, metadata);
        break;
      case WARN:
        Auralogs.warn(message, metadata);
        break;
      case ERROR:
        if (throwable != null) Auralogs.error(message, metadata, throwable);
        else Auralogs.error(message, metadata);
        break;
      default:
        Auralogs.info(message, metadata);
    }
